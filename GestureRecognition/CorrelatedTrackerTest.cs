...
{
    class CorrelatedTrackerTest
    {
        CorrelatedHandTracker tracker;
        int numMarkers;
         ...
         * */
        public CorrelatedTrackerTest(int numFramesPerBuffer, int numMarkers, int seed, double Q, double R)
        {
            ...
            tracker = new CorrelatedHandTracker(numMarkers, 1*Q, 1*R, 20, 0.4);
            tracker.init();
            ...
            for (int f = 0; f < bufferSize; ++f)
            {
                ...
                tracker.predict(dt);
                tracker.correct(measurements, dt);
